...
class PricingServiceTest {
    ...
    @Test
    @DisplayName("Calculate discounted price with 100% discount")
    void calculateDiscountedPrice_With100PercentDiscount() {
        // Arrange
        BigDecimal basePrice = new BigDecimal("100.00");
        BigDecimal discountPercent = new BigDecimal("100.0");
    ...
    @CsvSource({
        ...
        "100.00, 4.0, 85.00",  // Very high elasticity - max discount (15%)
    ...
    @ParameterizedTest
    @CsvSource({
        "100.00, 2.0, 200.00",  // Elastic demand (e>1)
        "100.00, 1.5, 150.00",  // Less elastic demand
        "100.00, 1.1, 110.00",  // Slightly elastic demand
        "100.00, 0.9, 108.00",  // Slightly inelastic demand
        "100.00, 0.5, 120.00",  // More inelastic demand
        "100.00, 0.0, 120.00"   // Completely inelastic demand
    })
    @DisplayName("Calculate optimal price based on elasticity")
    void calculateOptimalPrice(String basePriceStr, String elasticityStr, String expectedStr) {
        // Arrange
        BigDecimal basePrice = new BigDecimal(basePriceStr);
        BigDecimal elasticity = new BigDecimal(elasticityStr);
        BigDecimal expected = new BigDecimal(expectedStr);
        ...
        // Assert
        assertEquals(expected, result, "Optimal price calculation failed for elasticity " + elasticityStr);
    }
}
